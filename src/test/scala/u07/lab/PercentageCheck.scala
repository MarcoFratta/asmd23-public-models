...
class PercentageCheck extends AnyFunSuite:
  ...
  test("Percentage check of one trace") {
    ...
    val percentageOf = percentageOfTime(List(trace)).andThen(roundAt(1))
    percentageOf(B) should be(16.7)
    percentageOf(C) should be(50.0)
  }
  ...
  test("Percentage check of one trace with small time") {
    ...
    val percentageOf = percentageOfTime(List(trace))
    percentageOf(B) should be (2.3365909072E-12 +- 0.01)
  }
  ...
  test("Percentage check of two traces") {
    ...
    val percentageOf = percentageOfTime(List(trace1, trace2)).andThen(roundAt(1))
    percentageOf(B) should be(21.4)
    percentageOf(C) should be(42.9)
  }
